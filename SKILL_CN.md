...
| 命令 | 描述 | 示例 |
|------|------|------|
| `create-task` | 创建新任务（支持自动拆解为子任务） | `/vibe create-task "实现用户认证系统"` |
...
**参数**：
- `goal` - 高层目标或任务标题（必需）
- `--single` - 可选标志，指定创建单个任务而不是自动拆解

**执行步骤（自动拆解模式）**：
1. 检查 `.vibe/tasks/` 目录是否存在
2. 理解用户提供的高层目标
3. 分析目标，识别关键步骤和依赖关系
4. 将目标拆解为 3-7 个逻辑合理的子任务
   - 每个子任务应该是独立的、可验证的
   - 子任务之间有清晰的依赖关系
   - 子任务按执行顺序排列
5. 读取现有任务编号，找到最大值
6. 为每个子任务创建独立的任务文件（编号递增）
7. 可选：创建一个主任务文件，引用所有子任务
8. 显示任务拆解摘要
